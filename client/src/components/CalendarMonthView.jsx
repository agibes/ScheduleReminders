...
import {findMonth, updateItemStyles, handleDragOver, handleDragStart, handleDrop, dragElement} from '../utils';
...
const calendarMonthView = ({draggingItem, setDraggingItem, currentDateTime, monthInFocus, setMonthInFocus, eventItems, setEventItems, changeDate, setChangeDate, monthModNext, setMonthModNext, monthModPrev, setMonthModPrev, setShowUpdateTime}) => {
    ...
    <>
      ...
      <div id="calendarMonthComponent" className="component calendarComponent">
        <div id="arrowButtons">
          <button id="previous" onClick={() => {
            ...
            <p>{monthInFocus[15].toLocaleString('default', { month: 'long', year: 'numeric'})}</p>  
        ...
        {monthInFocus.map((dayOfMonth) => {
          ...
          return (
            <div className="dayContainer" id={dayOfMonth.toString()} onDrop={(e)=>handleDrop(e, draggingItem, eventItems, changeDate, setChangeDate, setEventItems, setShowUpdateTime)} onDragOver={handleDragOver}>
              <p className={`smallNumber ${new Date().getMonth() == dayOfMonth.getMonth() ? '' : 'notCurrentMonth'}`} >{dayOfMonth.toString().match(/\d+/)}</p>
